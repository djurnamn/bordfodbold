...
**"Not too easy to manipulate" together with "no users or passwords"** reads
as friction, not security. A PIN checked server-side on the hosted adapter,
...
no pub/sub plumbing, the free tier has no inactivity pause (Supabase's free
projects pause after seven days, which is a real risk for a demo someone
opens a week later), and deployment is one command. Playwright for the
...
- The local store: seeding, the PIN lock, cross-tab notification, domain
  errors leaving data untouched, reset.
...
- Optimistic updates in the provider; not needed at Convex's latency, but the
  seam is there.
- The design pass. The prototype has functional styling on the design
  system's tokens; the visual review is deliberately last.
