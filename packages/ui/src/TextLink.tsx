...
interface TextLinkProps extends ComponentPropsWithRef<'a'> {
...
export function TextLink(props: TextLinkProps) {
  const { color, nonInteractive, as, children, className, style, ref, ...rest } = props;

  const RootTag = (nonInteractive ? 'span' : (as ?? 'a')) as ElementType<ComponentPropsWithRef<'a'>>;

  const bem = useBem('DjuiTextLink');
